...
//FIXME esse teste parece não estar funcionando, testar!!
...
test("Deve receber token ao logar", () => {
    ...
    return app.services.user.save({name: "Waler", mail, password: "123456"})
...
test("Não deve autenticar usuário com senha errada", () => {
    ...
    return request(app).post("/auth/signin")
            .send({mail:"wrong@mail.com", password: "654321" })
            .then((res) => {
            expect(res.status).toBe(400)
            expect(res.body.error).toBe("Usuário ou senha inválido")
    })
